...
describe("User Endpoints", () => {
    ...
    beforeEach(async () => {
        ...
        const verifiedUser = await userService_1.prisma.user.create({
            data: {
        ...
        const unverifiedUser = await userService_1.prisma.user.create({
            data: {
        ...
        // Generate JWT tokens
        userToken = jsonwebtoken_1.default.sign({ userId: verifiedUser.id }, JWT_SECRET);
        unverifiedUserToken = jsonwebtoken_1.default.sign({ userId: unverifiedUser.id }, JWT_SECRET);
        // Create some test transactions for the verified user
        await userService_1.prisma.transaction.createMany({
            data: [
                {
                    userId: userId,
                    amount: 10000,
    ...
    describe("GET /api/user/profile", () => {
        ...
        it("should require authentication", async () => {
            const res = await (0, supertest_1.default)(server_1.default).get("/api/user/profile");
        ...
        it("should handle non-existent user", async () => {
            ...
            const tempUser = await userService_1.prisma.user.create({
                data: {
                    email: "temp@test.com",
                    password: "hashedpassword",
                    firstName: "Temp",
            ...
            const tempToken = jsonwebtoken_1.default.sign({ userId: tempUser.id }, JWT_SECRET);
            // Delete the user but keep the token
            await userService_1.prisma.user.delete({ where: { id: tempUser.id } });
            const res = await (0, supertest_1.default)(server_1.default)
                .get("/api/user/profile")
    ...
    describe("PATCH /api/user/profile", () => {
        ...
        it("should update user profile successfully", async () => {
            ...
            const updatedUser = await userService_1.prisma.user.findUnique({
                where: { id: userId },
    ...
    describe("GET /api/user/transactions", () => {
        ...
        it("should only return transactions for the authenticated user", async () => {
            ...
            const otherUser = await userService_1.prisma.user.create({
                data: {
            ...
            await userService_1.prisma.transaction.create({
                data: {
                    userId: otherUser.id,
                    amount: 1000,
                    type: client_1.TransactionType.DEPOSIT,
        ...
        it("should require authentication", async () => {
            const res = await (0, supertest_1.default)(server_1.default).get("/api/user/transactions");
            expect(res.statusCode).toEqual(401);
            expect(res.body).toHaveProperty("success", false);
        });
        it("should require email verification", async () => {
