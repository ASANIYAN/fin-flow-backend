...
import { Prisma, PrismaClient, TransactionType } from "../lib/prisma";
 ...
 */
export const processVerifiedDeposit = async (
  userId: string,
  ...
  while (currentRetry < MAX_RETRIES) {
    try {
      ...
      if (data.status !== "success" || verifiedAmount !== amount) {
        ...
        throw new Error(
          `Verification failed: Status is '${data.status}' or amount mismatch (${verifiedAmount} vs ${amount}).`
        );
      }
